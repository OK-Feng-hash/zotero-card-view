# Zotero Card View 0.3 Architecture
...
| `preferences.js` | Canonical preference keys, global-scope reads/writes, and one-time migration from the historical duplicated prefix. |
...
| `style-metrics-sync.js` | Waits for Zotero Style storage readiness, prioritizes missing visible-journal data through Style's public renderer, and reports cache changes for item-scoped invalidation. |
| `rating-adapter.js` | Reads Zotero Style ratings and their configured star appearance. |
| `reading-progress-adapter.js` | Read-only bridge to Ethereal Style's local per-page reading-time cache and title appearance preferences. |
| `view-position.js` | Captures and restores card-grid scroll position using both pixels and a visible-item anchor. |
| `sort-fields.js` | Canonical definitions, labels, defaults, and persistence normalization for configurable sort fields. |
| `sorter.js` | Pure ordering for the 15 supported fields, including journal partition and TOP precedence. |
...
4. `style-metrics-sync.js` watches the local Style rank cache and prioritizes missing metrics for cards inside the viewport.
5. `card-view.js` reads a lightweight reading-progress signature and reuses the existing DOM node when its summary, expansion state, and progress are unchanged.
6. `card-renderer.js` builds details only after the user expands a card.
...
- Zotero Style rank-cache change: invalidate only cards whose publication title uses that cache entry.
...
Expanded IDs and scroll position survive ordinary refreshes. Hidden renders are deferred while a PDF or another Zotero tab is active; returning to the library restores both the last pixel offset and the selected visible card's viewport offset. Selection synchronization updates only changed cards outside a render; a render performs one forced pass so newly created nodes always receive the correct state.
...
- Journal data is read from Zotero Style's local cache or the item's local `Extra` field. For missing metrics on visible cards, Card View invokes Style's public publication-tag renderer; Style alone decides whether and how to query its configured source.
- Zotero Style credentials are neither read nor stored, and Card View does not issue the metric network request itself.
- Reading progress is read only from Ethereal Style's already-loaded local `readingTime` cache and is never copied or written.
